...
exports.importExcel = async (req, res, next) => {
  try {
    ...
    for (const sheetName in parsedData) {
      ...
      if (sheetName.toLowerCase().includes('thuê phòng') || 
          sheetName.match(/P\d+-\d+\./i)) {
        // Rental sheet
      ...
      else if (sheetName.toLowerCase().includes('chi-tieu') || 
      ...
      else if (sheetName.toLowerCase().includes('deposit') || 
      ...
      else if (sheetName.toLowerCase().includes('setting') || 
