...
export interface IRentalPropertyModel extends Document {
...
export interface IRentalPropertyStatic extends Model<IRentalPropertyModel> {
  findByUserId(userId: mongoose.Types.ObjectId): Promise<IRentalPropertyModel[]>;
  findActiveProperties(userId: mongoose.Types.ObjectId): Promise<IRentalPropertyModel[]>;
}

// Create the schema
const rentalPropertySchema = createBaseSchema<IRentalPropertyModel>(
  {
    userId: {
...
// Add indexes
rentalPropertySchema.index({ userId: 'ascending', isActive: 'descending', createdAt: 'descending' });
rentalPropertySchema.index({ userId: 'ascending', roomCode: 'ascending' });

// Add static methods
rentalPropertySchema.statics.findByUserId = async function(userId: mongoose.Types.ObjectId) {
  return this.find({ userId, isDeleted: { $ne: true } }).sort({ isActive: -1, createdAt: -1 });
};

rentalPropertySchema.statics.findActiveProperties = async function(userId: mongoose.Types.ObjectId) {
  return this.find({ userId, isActive: true, isDeleted: { $ne: true } }).sort({ createdAt: -1 });
};

// Export the model
const RentalProperty: IRentalPropertyStatic = 
  mongoose.models.RentalProperty as IRentalPropertyStatic ||
  mongoose.model<IRentalPropertyModel, IRentalPropertyStatic>('RentalProperty', rentalPropertySchema);
